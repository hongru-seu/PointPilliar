...
def _points_to_bevmap_reverse_kernel(points,
                                     ...
                                     max_voxels=40000):
    """points to bevmap reverse kernel"""
    ...
    coor = np.zeros(shape=(ndim + 1,), dtype=np.int32)  # DHW
    voxel_num = 0
    for i in range(n):
        failed = False
        for j in range(ndim):
...
def points_to_bev(points,
    ...
    _points_to_bevmap_reverse_kernel(points, voxel_size, coors_range, coor_to_voxelidx,
                                     bev_map, height_lowers, with_reflectivity, max_voxels)
    return bev_map
