...
class DataBaseSampler:
    ...
    def sample_all(self, root_path, gt_boxes, gt_names, num_point_features,
                   random_crop=False, gt_group_ids=None, rect=None, trv2c=None, p2=None):
        """sample all"""
        sampled_num_dict = {}
                ...
                if sampled_cls:
                    ...
                    if self._use_group_sampling:
                        if len(sampled_cls) == 1:
                            ...
                            sampled_group_ids = np.stack([s["group_id"] for s in sampled_cls],
                                                         axis=0)
                        total_group_ids = np.concatenate([total_group_ids, sampled_group_ids],
                                                         axis=0)
        ...
        if sampled:
            ...
            s_points_list = []
            for info in sampled:
                s_points = np.fromfile(str(pathlib.Path(root_path) / info["path"]),
                                       dtype=np.float32)
                s_points = s_points.reshape([-1, num_point_features])
                if "rot_transform" in info:
                    rot = info["rot_transform"]
                    s_points[:, :3] = box_np_ops.rotation_points_single_angle(s_points[:, :3],
                                                                              rot, axis=2)
                s_points[:, :3] += info["box3d_lidar"][:3]
                s_points_list.append(s_points)
            if random_crop:
                s_points_list_new = []
                gt_bboxes = box_np_ops.box3d_to_bbox(sampled_gt_boxes, p2)
                ...
                for i in range(crop_frustums.shape[0]):
                    ...
                    mask = prep.mask_points_in_corners(s_points,
                                                       crop_frustums[i:i + 1]).reshape(-1)
        ...
        for i in range(num_gt, num_gt + num_sampled):
            if coll_mat[i].any():
                ...
                if self._enable_global_rot:
                    ...
                    sampled[i - num_gt]["rot_transform"] = (boxes[i, -1]
                                                            - sp_boxes[i - num_gt, -1])
    ...
    def sample_group(self, name, num, gt_boxes, gt_group_ids):
        ...
        valid_samples = []
        idx = num_gt
        for gnum in group_num:
            if coll_mat[idx:idx + gnum].any():
                coll_mat[idx:idx + gnum] = False
                coll_mat[:, idx:idx + gnum] = False
            else:
                for i in range(gnum):
                    if self._enable_global_rot:
