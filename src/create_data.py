...
def _calculate_num_points_in_gt(data_path, infos, relative_path, remove_outside=True, num_features=4):
    ...
    for info in infos:
        ...
        p2 = info['calib/P2']
        if remove_outside:
            points_v = box_ops.remove_outside_points(points_v, rect,
        ...
        gt_boxes_lidar = box_ops.box_camera_to_lidar(gt_boxes_camera, rect, trv2c)
        indices = box_ops.points_in_rbbox(points_v[:, :3], gt_boxes_lidar)
...
def create_groundtruth_database(data_path,
    ...
    for info in prog_bar(kitti_infos):
        ...
        trv2c = info['calib/Tr_velo_to_cam']
        if not lidar_only:
            points = box_ops.remove_outside_points(points, rect,
                                                   trv2c, p2, info["img_shape"])
        ...
        rbbox_lidar = box_ops.box_camera_to_lidar(rbbox_cam, rect, trv2c)
        ...
        point_indices = box_ops.points_in_rbbox(points, rbbox_lidar)
